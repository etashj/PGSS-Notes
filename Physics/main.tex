...
\end{align*}
\subsubsection*{Classical with new units}
\begin{align*}
    \beta &\ll 1\\
    P_c&=m\beta c\\
    E_c&=\frac{1}{2}m\beta^2 c^2\\
    P&=m\beta\\
    E&=\frac{1}{2}m\beta^2
\end{align*}
Using the two particles collision as a way to derive the relativistic momentum the equation setup is as follows. 
\begin{align*}
    C_1\beta_A + C_2\beta_B &= C_3\beta_A' + C_4\beta_B'\\
    \text{Let } C_i& \text{ be a function}\\
    C_1(m_A, \beta_A)\beta_A + C_2(m_B, \beta_B)\beta_B &
\end{align*}
\end{document}